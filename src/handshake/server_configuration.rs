...
use crypto::aead::{AeadAlgorithm, AEAD_ALGORITHM_COUNT};
...
#[derive(Debug)]
struct PublicKeyEntry<T>(T);

impl Readable for PublicKeyEntry<PublicKey> {
    fn read<R: Read>(reader: &mut R) -> Result<Self> {
        trace!("reading public key entry");

        let length = U24::read(reader)?;
        let length: u32 = length.into();
        trace!("determined public key entry is {} bytes long", length);
        ...
        let public_key_entry = PublicKeyEntry(bytes.as_slice().into());

        debug!("read public key entry {:?}", public_key_entry);

        Ok(public_key_entry)
    }
}

impl<T: Borrow<PublicKey> + Debug> Writable for PublicKeyEntry<T> {
    fn write<W: Write>(&self, writer: &mut W) -> Result<()> {
        trace!("writing public key entry {:?}", self);

        let bytes = self.0.borrow().bytes();

        let length = bytes.len() as u32;
        let length = U24::try_from(length).chain_err(|| ErrorKind::PublicKeyBytesTooLongForU24)?;
        ...
        debug!("written public key entry {:?}", self);
...
impl ServerConfiguration {
    ...
    pub fn from_tag_value_map(tag_value_map: &TagValueMap) -> Result<Self> {
        trace!(
            "building server configuration from tag value map {:?}",
            tag_value_map
        );

        let server_configuration_id = tag_value_map.get_required_value(Tag::ServerConfigurationId)?;
        let key_exchange_algorithms: SmallVec<
            [KeyExchangeAlgorithm; KEY_EXCHANGE_ALGORITHM_COUNT],
        > = tag_value_map.get_required_values(Tag::KeyExchangeAlgorithm)?;
        let aead_algorithms =
            tag_value_map.get_required_values(Tag::AuthenticatedEncryptionAlgorithm)?;
        let public_keys: SmallVec<[PublicKeyEntry<PublicKey>; AEAD_ALGORITHM_COUNT]> =
            tag_value_map.get_required_values(Tag::PublicValue)?;
        ...
        let public_keys: SmallVec<[PublicKey; AEAD_ALGORITHM_COUNT]> =
            public_keys.into_iter().map(|x| x.0).collect();

        let shared_key = {
            // Need to find the first key exchange algorithm we support
            let (public_key_index, key_exchange_algorithm, public_key): (
                usize,
                ...
                .next()
                .ok_or_else(|| {
                    Error::from(ErrorKind::ASupportedKeyExchangeAlgorithmMustBeSpecified)
                })?;
        ...
        let expiry_time = tag_value_map.get_required_value(Tag::ServerConfigurationExpiry)?;
        ...
        debug!(
            "built server configuration {:?} from tag value map {:?}",
    ...
    pub fn to_tag_value_map(&self) -> TagValueMap {
        trace!(
            "building tag value map from server configuration {:?}",
            self
        );
        ...
        let public_key_entries: SmallVec<[_; AEAD_ALGORITHM_COUNT]> = self.public_keys
            .iter()
        ...
        debug!(
            "built tag value map {:?} from server configuration {:?}",
            tag_value_map,
            self
        );
